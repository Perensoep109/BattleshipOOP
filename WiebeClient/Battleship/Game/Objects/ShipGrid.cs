...
namespace Battleship.Game.Objects
{
    class ShipGrid : BaseGrid
    {
        ...
        private ShipPreview m_shipPreview;
        public ShipGrid(Vector2 a_pos, int a_width, int a_height, int a_tileDim) : base(a_pos, a_width, a_height, a_tileDim)
        {
            ...
            m_shipPreview = new ShipPreview();
            m_shipPreview.m_length = 5;
            m_shipPreview.m_dir = new Vector2(1, 0);
        ...
        public override void Draw(SpriteBatch a_spriteBatch)
        {
            ...
            // Restrict X position
            if(m_shipPreview.m_pos.X >= m_pos.X && m_shipPreview.m_pos.X < m_pos.X + GridWidth * TileDim)
                // Restrict Y position
                if(m_shipPreview.m_pos.Y >= m_pos.Y && m_shipPreview.m_pos.Y <= m_pos.Y + GridHeight * TileDim)
                    m_shipPreview.Draw(a_spriteBatch);
        }

        public void CreateShip()
        {
            if (m_shipPreview.m_validPos)
                m_ships.Add(new Ship(GetGridCoord(m_shipPreview.m_pos), m_shipPreview.m_dir, m_pos, m_shipPreview.m_length));
        }

        public bool ValidateShipPos(Vector2 a_pos, Vector2 a_dir, int a_length)
        {
        ...
        {
            m_shipPreview.ScrollIndex = a_scrollVal;
            m_shipPreview.m_validPos = ValidateShipPos(m_shipPreview.m_pos, m_shipPreview.m_dir, m_shipPreview.m_length);
            m_shipPreview.m_pos = new Vector2((int)Math.Floor((decimal)(a_mouseX / TileDim)) * TileDim, (int)Math.Floor((decimal)(a_mouseY / TileDim)) * TileDim);
        }
